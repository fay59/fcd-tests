	...
	while (true)
	{
		...
		if (!((__zext uint32_t)(phi22 == 255) < ~phi25 | (phi20 & 1) == 0))
		{
			*(uint8_t*)(phi28 + 15) = (__zext uint8_t)(phi26 == 0);
			uint32_t* anon36 = (uint32_t*)(phi28 + 8);
			*anon36 = anon29;
			uint64_t anon37 = phi17 & 0xffffffff;
			main(4195952, phi28, (__zext uint64_t)phi21, arg3, phi16, phi20 << 32 >> 32 >> 1 & 0xffffffff, anon37);
			uint64_t anon38 = (__zext uint64_t)(*(uint32_t*)&alloca3 - anon35 - *anon36);
			alloca3 = anon38;
			main(4195982, phi28, anon38, arg3, 0, 0, anon37);
			main(4196002, phi28, anon38, arg3, phi16, 0, anon37);
		}
		break;
		...
		{
			uint32_t anon40 = printf((uint8_t*)0x400884);
			uint32_t anon41 = fflush(*(struct _IO_FILE**)0x600c30);
			phi_in39 = (__zext uint64_t)(anon40 + anon41);
		}
		uint64_t phi42 = phi_in39;
		uint32_t anon43 = *(uint32_t*)&alloca3 - 1;
		alloca3 = (__zext uint64_t)anon43;
		phi_in4 = phi16;
		phi_in5 = phi42;
		phi_in19 = anon34;
