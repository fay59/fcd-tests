	...
	*(uint16_t*)(anon8 + 18) = 65535;
	...
	if (anon6 >= 2)
	{
		...
		do
		{
			...
			while (true)
			{
				...
				if (dispatch20 == 1 || anon58 != 0 && dispatch20 == 2 && anon48 || anon58 != 0 && dispatch20 == 0 && anon48)
				{
					...
					*(uint16_t*)phi87 = (uint16_t)phi89 + (uint16_t)anon90 * 55536;
					...
					phi_in55 = phi102;
					phi_in56 = phi104;
					uint64_t anon105 = (uint64_t)anon90;
					phi_in57 = anon105;
					dispatch20 = 1;
					if (anon105 == 0)
					{
			...
			uint64_t phi106 = phi_in49;
			uint64_t phi107 = phi_in50;
			uint64_t phi108 = phi_in51;
			*(uint16_t*)phi108 = 65535;
			phi_in12 = phi106;
			phi_in13 = phi107;
			anon109 = phi24 & 0xffff;
			if (anon6 > anon109)
			{
				phi_in14 = phi25;
				phi_in15 = phi23;
	...
	uint64_t phi110 = phi_in12;
	uint64_t phi111 = phi_in13;
	uint64_t phi_in112 = phi110;
	uint64_t phi_in113 = 0;
	uint64_t phi_in114 = phi111;
	uint64_t phi_in115 = 0;
	uint64_t phi_in116 = 1;
	uint32_t dispatch117 = 0;
	if (*(uint16_t*)(phi111 + 16) != 65535)
	{
		uint64_t phi118;
		do
		{
			...
			phi_in115 = phi118;
			dispatch117 = 0;
			if (*(uint16_t*)(phi111 + (phi118 << 1) + 16) != 65535)
			{
				phi_in112 = phi110;
				phi_in113 = 0;
				phi_in114 = phi111;
		...
		while (*(uint16_t*)(phi111 + (phi118 << 1) + 16) != 65535);
	}
	do
	{
		uint64_t phi_in127;
		...
		while (true)
		{
			...
			if (dispatch117 == 1 || dispatch117 == 0 && anon124 > -1)
			{
				uint64_t phi128 = phi_in123;
				uint64_t phi129 = phi_in125;
				uint8_t* phi_in130 = (uint8_t*)0x400a44;
				uint64_t phi_in131 = 1;
				if (phi129 != 0)
				{
					phi_in130 = (uint8_t*)0x400a48;
					phi_in131 = 20;
					if (phi129 != ((uint64_t)((__zext uint128_t)phi129 * 970881267037344822 >> 64) - (phi129 >> 63)) * 19)
					{
						phi_in130 = (uint8_t*)0x400a49;
						phi_in131 = phi129 + 1;
					}
				}
				uint8_t* phi132 = phi_in130;
				uint64_t phi133 = phi_in131;
				printf(phi132);
				uint64_t anon134 = phi128 - 1;
				phi_in123 = anon134;
				phi_in125 = phi133;
				phi_in126 = phi133;
				phi_in127 = phi119;
		...
		uint64_t phi135 = phi_in126;
		phi136 = phi_in127;
		phi_in112 = *(uint64_t*)(phi136 + 8);
		phi_in113 = phi135;
		phi_in114 = *(uint64_t*)(phi136 + 8);
		phi_in115 = 1000;
		dispatch117 = 0;
	}
	while (*(uint64_t*)(phi136 + 8) != 0);
	_IO_putc(10, *anon5);
	return;
}
