...
void main(uint64_t arg0, uint64_t arg1)
{
	...
	while (true)
	{
		...
		uint64_t phi_in30 = phi13;
		uint64_t phi31 = phi14;
		uint64_t phi32 = phi_in15;
		uint64_t phi33 = phi_in30;
		uint64_t phi34 = 1;
		uint32_t phi35 = 1;
		uint64_t anon36 = m.constprop.1(4195713, (__zext uint64_t)phi16, (__zext uint64_t)phi35);
		...
		if (phi43 != 0)
		{
			if ((uint32_t)m.constprop.1(4196072, phi33 & 0xffffffff, 0) != 0)
			{
				uint32_t anon51;
				putchar(10);
				uint64_t phi45 = 65;
